...
export function BookAppointmentModal({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      providersApi.getProviders().then(res => {
        if (res.success) setProviders(res.data);
      });
    }
  }, [isOpen, prefillPatientId, prefillServiceId]);

  // Handle slot fetching when step 3 starts or date/provider changes
  useEffect(() => {
    if (step === 3) {
      // Mock fetching slots
      setSlots([
        { start: "09:00", available: true },
        { start: "09:30", available: false, reason: "Booked" },
        { start: "10:00", available: true },
        { start: "10:30", available: true },
        { start: "11:00", available: true },
        { start: "11:30", available: false, reason: "Break" },
      ]);
    }
  }, [step, date, provider, autoAssign]);
  ...
  const handleBook = async () => {
    ...
    try {
      const payload = {
        ...
        provider_id: autoAssign ? undefined : provider.id,
      ...
      if (rescheduleAppointmentId) {
        // await appointmentsApi.reschedule(rescheduleAppointmentId, payload);
      ...
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-3xl bg-white rounded-2xl shadow-2xl z-50 flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-white relative z-10">
          <div>
            ...
            {rescheduleAppointmentId && (
              <span className="inline-block mt-1 px-2 py-0.5 bg-amber-100 text-amber-700 text-[10px] font-bold rounded uppercase tracking-wider">
                Rescheduling Flow
              </span>
            )}
          </div>
        ...
        <div className="p-6 overflow-y-auto hidden-scrollbar flex-1 bg-white">
            ...
            <div className="space-y-6 animate-in slide-in-from-right-8 duration-300">
              ...
              {patient ? (
                <div className="p-4 border border-blue-200 bg-blue-50 rounded-xl flex items-center justify-between">
                  ...
                  <button onClick={() => setPatient(null)} className="text-xs font-bold text-blue-600 hover:underline">
                    Change
                  </button>
                </div>
              ) : (
                <PatientSearchInput 
                  onSelect={setPatient} 
                  autoFocus 
                  onCreateNew={() => toast.info("Inline patient creation not implemented yet")}
                />
              )}
            </div>
          )}
                  ...
                  {services.map(s => (
                    <label 
                      ...
                      className={cn(
                        ...
                        service?.id === s.id ? "border-blue-600 bg-blue-50/50 shadow-sm" : "border-slate-200 hover:border-blue-300"
                      )}
                    >
                      <input 
                        type="radio" 
                ...
                <div className="flex gap-3">
                  {(["standard", "urgent", "emergency"] as const).map(p => (
                    <label 
                      key={p} 
                      className={cn(
                        "flex-1 text-center p-3 rounded-xl border cursor-pointer transition-all",
                        priority === p ? (
          ...
          {/* STEP 3: PROVIDER & SLOTS */}
          {step === 3 && (
            <div className="space-y-6 animate-in slide-in-from-right-8 duration-300">
              <div className="flex items-center justify-between p-4 bg-slate-50 border border-slate-200 rounded-xl">
                <div>
                  <h4 className="font-bold text-slate-800 text-sm">Provider Assignment</h4>
                  <p className="text-xs text-slate-500">Auto-assign to the first available provider or select specifically.</p>
                </div>
                <label className="flex items-center gap-2 cursor-pointer">
                  <div className="relative">
                    <input type="checkbox" className="sr-only" checked={autoAssign} onChange={e => { setAutoAssign(e.target.checked); setProvider(null); }} />
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-48 overflow-y-auto">
                    {providers.map(p => (
                      <label 
                        ...
                        className={cn(
                          "flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all",
                          provider?.id === p.id ? "border-blue-600 bg-blue-50/50 shadow-sm" : "border-slate-200 hover:border-blue-300"
                        )}
                      >
                        <input type="radio" className="sr-only" checked={provider?.id === p.id} onChange={() => setProvider(p)} />
                        <div className={cn("w-4 h-4 rounded-full border flex items-center justify-center", provider?.id === p.id ? "border-blue-600" : "border-slate-300")}>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {slots.map((slot, idx) => (
                      <button
                        ...
                        className={cn(
                          "py-2 rounded-lg text-sm font-bold transition-all border text-center",
                          !slot.available ? 'bg-slate-50 border-slate-100 text-slate-400 cursor-not-allowed opacity-60' : 
                          timeSlot === slot.start ? 'bg-blue-600 border-blue-600 text-white shadow-md' : 'bg-white border-slate-200 text-slate-700 hover:border-blue-400'
                        )}
                        title={slot.reason}
                      >
                        {slot.start}
          ...
          {step === 4 && (
            <div className="space-y-6 animate-in slide-in-from-right-8 duration-300">
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-6 text-center">
                ...
                <p className="text-sm text-slate-500">Please review the details below before completing.</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-y-4 gap-x-8 px-4">
                <div className="flex flex-col gap-1">
                  ...
                  <span className="text-sm font-bold text-slate-900 flex items-center gap-2"><User className="w-3.5 h-3.5 text-slate-400" /> {patient?.name}</span>
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Service</span>
                  <span className="text-sm font-bold text-slate-900 flex items-center gap-2"><Stethoscope className="w-3.5 h-3.5 text-slate-400" /> {service?.name}</span>
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Date & Time</span>
                  <span className="text-sm font-bold text-slate-900 flex items-center gap-2"><Clock className="w-3.5 h-3.5 text-slate-400" /> {date} at {timeSlot}</span>
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Provider</span>
                  <span className="text-sm font-bold text-slate-900">{autoAssign ? "Auto Assigned" : provider?.full_name}</span>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-between bg-white rounded-b-2xl">
          <button 
            ...
            className={cn("px-4 py-2 text-sm font-bold text-slate-600 hover:bg-slate-100 rounded-xl transition-colors disabled:opacity-0")}
          >
            Back
          </button>
          ...
          {step < 4 ? (
            <button 
              ...
              disabled={
                (step === 1 && !patient) || 
                (step === 2 && !service) || 
                (step === 3 && (!timeSlot || (!autoAssign && !provider)))
              }
              className="flex items-center gap-2 px-6 py-2 bg-slate-900 text-white text-sm font-bold rounded-xl shadow-md disabled:opacity-50 hover:bg-slate-800 transition-colors"
            >
            ...
            <button 
              onClick={handleBook}
              disabled={loading}
              className="flex items-center gap-2 px-8 py-2 bg-blue-600 text-white text-sm font-bold rounded-xl shadow-md shadow-blue-500/20 disabled:opacity-50 hover:bg-blue-700 transition-colors"
            >
